...
private:
	CFbxMod* alistar[Alistar_Animation];
	GLuint state;
	...
	AttackData attackData[AlistarAttackNum];	//Attack data
...
public:
	...
	//Get the attack power
	GLfloat GetAttackPower() { if(state == Alistar_Attack1) { return attackData[AlistarAttack1].AttackPower;} if(state == Alistar_Attack2) { return attackData[AlistarAttack2].AttackPower;} if(state == Alistar_Crit) { return attackData[AlistarAttackCrit].AttackPower;} return 0.0; }
	GLboolean GetAttackState() { if(state == Alistar_Attack1) { return attackData[AlistarAttack1].Attacking;} if(state == Alistar_Attack2) { return attackData[AlistarAttack2].Attacking;} if(state == Alistar_Crit) { return attackData[AlistarAttackCrit].Attacking;} return GL_FALSE; }
	GLboolean GetAttackTiming() { if(state == Alistar_Attack1) { return attackData[AlistarAttack1].AttackTiming;} if(state == Alistar_Attack2) { return attackData[AlistarAttack2].AttackTiming;} if(state == Alistar_Crit) { return attackData[AlistarAttackCrit].AttackTiming;} return GL_FALSE; }
