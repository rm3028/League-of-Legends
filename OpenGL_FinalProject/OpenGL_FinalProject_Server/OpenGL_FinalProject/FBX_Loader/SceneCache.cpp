...
void VBOMesh::Draw(int pMaterialIndex, ShadingMode pShadingMode) const
{
    ...
    if ( pShadingMode == SHADING_MODE_SHADED)
    {
        ...
        glDrawElements(GL_TRIANGLES, lElementCount, GL_UNSIGNED_INT, reinterpret_cast<const void *>(lOffset));
    }
    else
    {
        for (int lIndex = 0; lIndex < mSubMeshes[pMaterialIndex]->TriangleCount; ++lIndex)
        {
            ...
            glDrawElements(GL_LINE_LOOP, TRIANGLE_VERTEX_COUNT, GL_UNSIGNED_INT, reinterpret_cast<const void *>(lOffset));
