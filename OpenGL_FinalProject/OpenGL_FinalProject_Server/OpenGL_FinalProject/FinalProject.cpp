...
namespace FinalProject {
	...
	CFinalProject::~CFinalProject(GLvoid)
	{
		...
		if(gameState >= GameLoading) {
			socket_server->close();
			delete server,client,sceneMap, camera,socket_server;
		}
	...
	GLvoid CFinalProject::OnCreate(int argc, char* argv[])
	{
		...
		glutCreateWindow("成圖技術與應用 Final Project Server");
	...
	GLvoid CFinalProject::OnTimer(int id)
	{
		...
		case 0:
			...
			if ((currentTime - lastUpdate) >= 1000 ) {
				sprintf_s(title, "成圖技術與應用 Final Project Server   FPS: %d", frames);
	...
	GLvoid CFinalProject::OnDisplay(GLvoid)
	{
		...
		case GamePlay: {
			...
			//static thread SocketThread(SetSocketServerThread);
			...
			client->getSelectCharacter()->OnDisplayHeadLife(camera->GetAngleY());

			server->getSelectCharacter()->OnDisplayLife();
			...
			glPushMatrix();
				...
				glPushMatrix();
					...
					sceneMap->SetMonAerialLoc(*client->getSelectCharacter()->GetLocation(), client->getSelectCharacter()->GetLocAngle() + client->getSelectCharacter()->GetmoveAngle());	//Render monster's location on aerial map
					sceneMap->SetMasAerialLoc(*server->getSelectCharacter()->GetLocation(), server->getSelectCharacter()->GetLocAngle() + server->getSelectCharacter()->GetmoveAngle());	//Render character's location on aerial map
				glPopMatrix();
				glMatrixMode(GL_PROJECTION);
			glPopMatrix();
	...
	GLvoid CFinalProject::OnMouseFunc(int button, int state, int x, int y) 
	{
		...
		case GamePlay:
			...
			{
				server->getSelectCharacter()->Attack();
			}
			else if(button == GLUT_RIGHT_BUTTON && state == GLUT_DOWN)
			{
				server->getSelectCharacter()->Avoid();
			}
	...
	void  CFinalProject::OnMousePassiveMotion(int x, int y)
	{
		...
		case GamePlay:
			if(mouseLock) {
				...
				if(server->getSelectCharacter()->GetState() > Akali_Idle1)
					server->getSelectCharacter()->SetLocAngle(camera->GetAngleY());
			}
			break;
		}
	}
	...
	GLvoid CFinalProject::OnKeyboardFunc(unsigned char key, int x, int y) 
	{
		...
		case GamePlay:
			...
			{
			case 'w':case 'W':
				server->getSelectCharacter()->MoveFront(GL_TRUE);
				break;

			case 'a':case 'A':
				server->getSelectCharacter()->MoveLeft(GL_TRUE);
				break;

			case 's':case 'S':
				server->getSelectCharacter()->MoveBack(GL_TRUE);
				break;

			case 'd':case 'D':
				server->getSelectCharacter()->MoveRight(GL_TRUE);
				break;
	...
	GLvoid CFinalProject::OnKeyboardUpFunc(unsigned char key, int x, int y) 
	{
		...
		case GamePlay:
			...
			{
			case 'w':case 'W':
				server->getSelectCharacter()->MoveFront(GL_FALSE);
				break;
			case 'a':case 'A':
				server->getSelectCharacter()->MoveLeft(GL_FALSE);
				break;
			case 's':case 'S':
				server->getSelectCharacter()->MoveBack(GL_FALSE);
				break;
			case 'd':case 'D':
				server->getSelectCharacter()->MoveRight(GL_FALSE);
				break;
			case ' ':
				//server->getSelectCharacter()->SetLocAngle(camera->GetAngleY());
				while(ShowCursor(TRUE) < 0)
	...
	GLvoid CFinalProject::LoadThreadFunc(GLvoid)
	{
		...
		camera->SetFollow(server->getSelectCharacter()->GetLocation());
		...
		socket_server->sendCharacter(charSelect);
		socket_server->receiveCharacter();

		client=new Client(socket_server->character);
		...
		//等待兩端loading完第二隻
		socket_server->getReady();
		SocketThread = thread(SetSocketServerThread);
		gameState = GamePlay;
		playWAV(".\\res\\SAOBGM.wav", true);
	}

	
	GLvoid CFinalProject::SocketServerThreadFunc(GLvoid){
		while(!exitGame){
			socket_server->sendCoordinate(server->getSelectCharacter()->GetLocation()->x
				,server->getSelectCharacter()->GetLocation()->y
				,server->getSelectCharacter()->GetLocation()->z
				,server->getSelectCharacter()->GetState()
				,server->getSelectCharacter()->GetLocAngle()
				,server->getSelectCharacter()->GetmoveAngle()
				,server->getSelectCharacter()->GetMod()->GetCurrentFrame()
				,client->getSelectCharacter()->GetHealth()
				,client->getSelectCharacter()->GetDeath());
			
			socket_server->receiveCoordinate();
			client->getSelectCharacter()->SetLocation(socket_server->x,socket_server->y,socket_server->z);
			client->getSelectCharacter()->SetState(socket_server->state);
			client->getSelectCharacter()->setControl_Mode(false);
			client->getSelectCharacter()->SetLocAngle(socket_server->locAngle);
			client->getSelectCharacter()->SetmoveAngle(socket_server->moveAngle);
			server->getSelectCharacter()->SetHealth(socket_server->health);
			server->getSelectCharacter()->SetDeath(socket_server->death);
		}
	}
	...
	GLvoid CFinalProject::SetSocketServerThread() 
	{
		instance->SocketServerThreadFunc();
	}
